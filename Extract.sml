...
struct
  ...
  and
    ...
    case subst of
      ...
      | BS_If((BP_list(pre), sub) :: ifs)                           => if pre <> [] andalso ifs = nil then (settle_constraints targetvar prelist anylist (iflist @ pre) sub) @ [PGInfo("skip", prelist, anylist, (iflist @ [BE_Node1(NONE, (Reserved "not"), (predicatelist_2_logicaland pre))]), BS_BecomesEqual (BE_Leaf(NONE, Var [targetvar]), BE_Leaf(NONE, Var [targetvar])))]
      ...
      | BS_Select((BP_list(pre), sub) :: ifs)                       => if pre <> [] andalso ifs = nil then (settle_constraints targetvar prelist anylist (iflist @ pre) sub) @ [PGInfo("skip", prelist, anylist, (iflist @ [BE_Node1(NONE, (Reserved "not"), (predicatelist_2_logicaland pre))]), BS_BecomesEqual (BE_Leaf(NONE, Var [targetvar]), BE_Leaf(NONE, Var [targetvar])))]
                                                                       else if pre = [] then settle_constraints targetvar prelist anylist iflist sub
