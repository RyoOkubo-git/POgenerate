...
struct
  ...
  and (* val parse : BToken list -> BComponent *)
    parse ((Reserved "IMPLEMENTATION")::(Var [machinename])::LBracket::btokens) = 
      let
        ...
        val (rn, btkns) = p_refines r
      in
        BImp (machinename, rn, v, (p_clauses (rmend btkns)))
      end

    | parse ((Reserved "IMPLEMENTATION")::(Var [machinename])::btokens) = 
        let
          val (rn, btkns) = p_refines btokens
        in
          BImp (machinename, rn, [], (p_clauses (rmend btkns)))
        end
    | parse _ = raise ParserParseError "parse error in MACHINE parameter"
  and
    p_refines ((Reserved "REFINES") :: (Var [machinename]) :: btokens) = (machinename, btokens)
    | p_refines _ = raise ParserParseError "missing REFINES"
  and
      ...
      let
        ...
        fun clauses_sub [] = []
          | clauses_sub btkslst =
            ...
            | (Reserved "IMPORTS")::rest            => ("IMPORTS",p_imports rest) :: (clauses_sub (tl btkslst))
            ...
            | (Reserved "VALUES")::rest             => ("VALUES",p_values rest) :: (clauses_sub (tl btkslst))
    ...
    p_values btokens = 
      ...
      in
        if rest = [] then
          ...
          raise ParserParseError "extra tokens in VALUES clause"
      end
  and
  ...
  and
    p_imports btokens = BC_IMPORTS(p_machine_list btokens)
  and
